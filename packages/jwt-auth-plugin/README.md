...
`me`) плагин обслуживает сам поверх того же инстанса.
...
`token.access.subKey`), запрашивает `/me`. Пока восстановление не завершено,
`isReady` = `false`, а гард маршрутов ждёт его перед первой навигацией — это окно
удобно показать пользователю как состояние загрузки.
